...
class PetFriends:
    ...
    def addphoto_pet(self, auth_key, pet_id, pet_photo):
        """Метод отправляет на сервер запрос на добавлене фото питомца по указанному ID и возвращает
        статус запроса и результат в формате JSON с текстом уведомления .
         """
        data = MultipartEncoder(
            fields={
                'pet_photo': (pet_photo, open(pet_photo, 'rb'), 'image/jpeg')
